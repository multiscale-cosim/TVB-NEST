...
REPERTORY_LIB_NEST=${PWD}/../../lib/nest_run_3/lib/python3.6/site-packages/ # folder with py-nest
...
folder="./files/nb_VP_"${nb_VP}"nb_mpi"${nb_mpi}'_D_'${spike_generator}'_P_'${parrot}'_N_'${iaf}'_R_'${nb_mpi_recorder}'_Separate_'${separate}'_GS_'${nb_mpi_generator_spike}'_GC_'${nb_mpi_generator_current}'_SH_'${shared_mpi_input}'_SM_'${mix_mpi}

if [ -d $folder ]
then
  return
else
  sleep 5 # wait for the time of synchronization
  mkdir ${folder}
  mkdir ${folder}'/file_record/'
  mkdir ${folder}'/file_gen_spike/'
  mkdir ${folder}'/file_gen_current/'
  sleep 5 # wait for creation of the folder
  ...
  $RUN -n $nb_mpi python3 ./nest_config.py ${nb_VP} ${nb_mpi} ${nb_run} ${time_sim} ${spike_generator} ${parrot} ${iaf} ${nb_mpi_recorder} ${separate} ${nb_mpi_generator_spike} ${nb_mpi_generator_current} ${shared_mpi_input} ${mix_mpi}
