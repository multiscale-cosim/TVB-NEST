...
def print_tvb_figure(param, begin, end, spikes_ex, spikes_in, TVB_data):
    ...
    ax_rate_compare = fig.add_subplot(grid[1, 0])
    grid_ECOG = gridspec.GridSpecFromSubplotSpec(1, 2, subplot_spec=grid[2:, 0], hspace=0.01, wspace=0.1)
    ...
    for index, ax in enumerate([ECOG_1, ECOG_2]):
        ...
        for i in range(8):
            ax.plot(ECOG_time, ECOG[:, i + index] + i * max_ecog, linewidth=0.1)
