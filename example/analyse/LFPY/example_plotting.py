...
# This file is a modify version of the code from HybridLFPy:
# https://github.com/INM-6/hybridLFPy/blob/master/examples/example_plotting.py
...
def plot_pop_scatter(ax, somapos, isometricangle, marker, color):
    ...
    :param somapos: soma position of the neurons
    :param isometricangle: pseudo-3d view angle
    :param marker: the symbol for the neurons
...
def plot_population(ax,
    ...
    :param ax : matplotlib.axes.AxesSubplot
    :param aspect : str : matplotlib.axis argument
    :param isometricangle : float : pseudo-3d view angle
    :param plot_somas : bool : plot soma locations
    :param plot_morphos : bool : plot full morphologies
    :param num_unitsE : int : number of excitatory morphos plotted per population
    :param num_unitsI : int : number of inhibitory morphos plotted per population
    :param clip_dendrites : bool : draw dendrites outside of axis
    :param mainpops : bool :if True, plot only main pops, e.g. b23 and nb23 as L23I
    ...
    x_0 = np.array(populationParams['ex']['min_r'])[1, 1:-1]
    z_0 = np.array(populationParams['ex']['min_r'])[0, 1:-1]
...
def plot_morphologies(ax, X, isometricangle, markers, colors,
    ...
    """
    plot morphologies of the neurons and the electrode
    :param ax: matplotlib.axes.AxesSubplot
    :param X: str : name of population
    :param isometricangle:  float : pseudo-3d view angle
    :param markers: array of str : the symbols for the neurons
    :param colors: array of color : the colors
    :param populations_path:  str : path to the file of the population
    :param cellParams: parameter of the neurons
    :param fraction: fraction of plotting neurons
    :return:
    """
    for (pop, marker, color) in zip(X, markers, colors):
        # get the somapos
...
def plot_individual_morphologies(ax, X, isometricangle, markers, colors,
    ...
    :param colors: array of color : the colors
    :param cellParams: parameter of the neurons
    :param populationParams: parameter of the population
    :return:
    """
    depth = []
    offsets = []
...
def plot_correlation(z_vec, x0, x1, ax, lag=20., title='firing_rate vs LFP'):
    ...
    :param x1 : second dataset - e.g., the multichannel LFP
    :param ax : matplotlib.axes.AxesSubplot object
    :param title : text to be used as current axis object title
    """
    zvec = np.r_[z_vec]
    zvec = np.r_[zvec, zvec[-1] + np.diff(zvec)[-1]]
