...
def run_exploration(path,begin,end,time_syn):
    parameter_test.param_co_simulation['co-simulation']=True
    parameter_test.param_co_simulation['synchronization']=time_syn
    ...
    run_exploration_2D(path+'/'+str(time_syn)+'/', parameter_test, {'g':np.arange(1.0, 1.2, 0.5), 'mean_I_ext': [0.0]}, begin, end)

if __name__ == "__main__":
    import sys
    if len(sys.argv)==5:
        run_exploration(sys.argv[1],float(sys.argv[2]),float(sys.argv[3]),float(sys.argv[4].replace(',','.')))
    elif len(sys.argv)==1:
