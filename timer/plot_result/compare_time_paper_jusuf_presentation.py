...
def full_figure(list_nb, time_sim, time_nest_sim, time_nest_IO, time_nest_wait,
    ...
    plt.subplots_adjust(left=0.13, right=0.97, top=0.99, wspace=wspase, hspace=0.1, bottom=0.07)
    return fig
...
if __name__ == '__main__':
    ...
    for index_run, ((folder, list_nb, label), folder) in enumerate(zip(folders_list, folders)):
        ...
        fig = full_figure(list_nb, time_sim, time_nest_sim, time_nest_IO, time_nest_wait,
                    time_nest_tot, time_TVB_sim, time_TVB_IO, time_TVB_tot,
                    label, wspase=wspase,
                    labelsize=20, labellegend=18, fontsize=18, figsize=(8, 8))
        # plt.show()
        plt.savefig(path_global + '/../../data/figure/timer/jusuf_node_presentation_mean.pdf', dpi=300)
        plt.savefig(path_global + '/../../data/figure/timer/jusuf_node_presentation_mean.png', dpi=300)
        plt.close('all')
