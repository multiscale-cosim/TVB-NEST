...
from nest_elephant_tvb.Tvb.modify_tvb.Interface_co_simulation import Interface_co_simulation, ReducedWongWang_proxy
...
def tvb_model(dt, weight, delay, id_proxy):
    ...
    populations = ReducedWongWang_proxy()
...
def tvb_init(dt, parameters, time_synchronize, initial_condition):
    ...
    :return:
        ...
        (weights_out,delay_out): the connectivity of disconnect region output
    """
    ...
    monitors = (lab.monitors.Raw(period=dt, variables_of_interest=np.array(0)),
                Interface_co_simulation(period=dt, id_proxy=np.asarray(id_proxy, dtype=np.int),
...
def tvb_simulation(time, sim, data_proxy):
    ...
    time = result[0][0]
    rate = np.concatenate(result[1][1][:, 0])
    s = result[0][1][:, 0]
    return time, rate, s


class TvbSim:
    def __init__(self, weight, delay, id_proxy, resolution_simulation, resolution_record, time_synchronize,
