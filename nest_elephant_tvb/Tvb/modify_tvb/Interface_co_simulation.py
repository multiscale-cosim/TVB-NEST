...
class ModelWithProxy:
    ...
    def update_proxy(self, data):
        ...
        :param data: valeur for the proxy node
        """
        self._update = True
...
class HistoryProxy(BaseHistory):
    ...
    buffer = NDArray(('n_time', 'n_cvar', 'n_node', 'n_mode'), 'f8',
                     read_only=False)  # the buffer of value of the proxy
...
# Modification of the Reduced Wong Wang in order to accept proxy value and to return the firing rate of the different
# node
...
class ReducedWongWang_proxy(ReducedWongWang, ModelWithProxy):
    """
...
class Interface_co_simulation(Raw):
    ...
    def config_for_sim(self, simulator):
        ...
        if hasattr(simulator.model, '_id_proxy'):
            ...
            if self._model_with_proxy is None:
                ...
                class model_proxy(model_class, ModelWithProxy):
                    """
        ...
        simulator.integrator.history_proxy = HistoryProxy(self.time_synchronize, simulator.integrator.dt, self.id_proxy,
                                                          simulator.model.cvar, simulator.model.number_of_modes)
    ...
    def sample(self, step, state):
        ...
        result[:] = [numpy.expand_dims(self.model.coupling_variable(), 0),
                     self.coupling(step + self._nb_step_time)[:, self.id_proxy, :]]
        return [time, result]
