...
def run(parameters_file):
    ...
    logger = create_logger(results_path, 'launcher', level_log)
    ...
    if param_co_simulation['co-simulation']:
        ...
        if not os.path.exists(results_path + '/transformation'):
            os.makedirs(results_path + '/transformation')
        # create all the repertory for the transformation file (communication files of MPI)
        if not os.path.exists(results_path + "/transformation/spike_detector/"):
            os.makedirs(results_path + "/transformation/spike_detector/")
        if not os.path.exists(results_path + "/transformation/send_to_tvb/"):
            os.makedirs(results_path + "/transformation/send_to_tvb/")
        # create all the repertory for the transformation file (communication files of MPI)
        if not os.path.exists(results_path + "/transformation/spike_generator/"):
            os.makedirs(results_path + "/transformation/spike_generator/")
        if not os.path.exists(results_path + "/transformation/receive_from_tvb/"):
            os.makedirs(results_path + "/transformation/receive_from_tvb/")
        if parameters['param_TR_nest_to_tvb']['save_hist']:
            if not os.path.exists(results_path + '/transformation/TSR_hist/'):
                os.mkdir(results_path + '/transformation/TSR_hist/')
        if parameters['param_TR_nest_to_tvb']['save_rate']:
            if not os.path.exists(results_path + '/transformation/TSR_rate/'):
                os.mkdir(results_path + '/transformation/TSR_rate/')
        if parameters['param_TR_tvb_to_nest']['save_spike']:
            if not os.path.exists(results_path + '/transformation/TRS_spike/'):
                os.mkdir(results_path + '/transformation/TRS_spike/')
        if parameters['param_TR_tvb_to_nest']['save_rate']:
            if not os.path.exists(results_path + '/transformation/TRS_rate/'):
                os.mkdir(results_path + '/transformation/TRS_rate/')
        ...
        nb_mpi_transformer = 1 if param_co_simulation['transformation_thread'] else 3
        for index in range(len(id_proxy)):
            if 'singularity' in param_co_simulation.keys():  # run with singularity image
                argv = mpirun + ['-n', str(nb_mpi_transformer), 'singularity', 'run',
                                 '--app', 'NEST-TVB', param_co_simulation['singularity']]
            elif 'sarus' in param_co_simulation.keys():  # run with docker
                argv = mpirun + ['-n', str(nb_mpi_transformer)] + param_co_simulation['sarus'] \
                       + ['python3', '/home/nest_elephant_tvb/transformation/nest_to_tvb.py']
            else:  # run local or with slurm
        ...
        # create transform between TVB to Nest:
        ...
        for index in range(len(id_proxy)):
            if 'singularity' in param_co_simulation.keys():  # run with singularity image
                ...
                argv = mpirun + ['-n', str(nb_mpi_transformer)] + param_co_simulation['sarus'] \
                       + ['python3', '/home/nest_elephant_tvb/transformation/tvb_to_nest.py']
            else:  # run local or with slurm
...
def run_exploration_2D(path, parameter_default, dict_variables, begin, end):
    ...
    for variable_1 in dict_variables[name_variable_1]:
        for variable_2 in dict_variables[name_variable_2]:
            # try:
            print('SIMULATION : ' + name_variable_1 + ': ' + str(variable_1) + ' ' + name_variable_2 + ': ' + str(
                variable_2))
            results_path = path + '_' + name_variable_1 + '_' + str(variable_1) + '_' + name_variable_2 + '_' + str(
                variable_2)
